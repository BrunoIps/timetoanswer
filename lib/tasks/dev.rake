namespace :dev do
  ...
  task setup: :environment do
    if Rails.env.development?
      show_spinner("Apagando BD") { %x(rails db:drop:_unsafe) } #aqui poderia usar "do" e "end" mas como é so uma linha de comando pode se usar {} nos lugares de "do" e "end"
      show_spinner("Criando BD") { %x(rails db:create)    } 
      show_spinner("Migrando BD") { %x(rails db:migrate) }
      show_spinner("Cadastrando ADM padrão") { %x(rails dev:add_default_admin) }
      show_spinner("Cadastrando ADMs extras") { %x(rails dev:add_extra_admins) }
  ...
  task add_answers_and_questions: :environment do
    Subject.all.each do |subject|
      rand(5..10).times do |i|
        params = create_question_params(subject = Subject.all.sample)
